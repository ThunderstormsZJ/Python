...
class FileUtils(object):
	...
	#
	@staticmethod
	def copy_file_with_ignore(src, dst, ignoreFiles=None, ignoreFloders=None, isPrint=True):
		...
		def __checkIgnoreFloder(floder, ignoreFloders):
			...
			for ignoreFloder in ignoreFloders:
				if os.path.isabs(ignoreFloder) and file.rfind(ignoreFloder)>=0:
					return True
				elif os.path.split(floder)[1] == ignoreFloder:
					return True
			return False
		...
		for (root,dirs,files) in os.walk(src):
			...
			if __checkIgnoreFloder(root, ignoreFloders):
				continue
			for d in dirs:
