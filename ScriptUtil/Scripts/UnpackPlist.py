...
@PluginDesAttribute(name="unpack_plist", description=getCurString(u"[用于解析Plist文件]"))
    ...
    def gen_png_from_plist(self, plist_filename, png_filename):
        ...
        for k, v in plist_dict['frames'].items():
           ...
           # 其他结构 plist
            ...
            kPathList = k.split("/")
            if len(kPathList) > 0:
                outfile = os.path.join(file_path, *kPathList[:-1])
                if not os.path.exists(outfile):
                    os.makedirs(outfile)
                outfile = os.path.join(outfile, kPathList[-1])
            else:
                outfile = os.path.join(outfile, k)
